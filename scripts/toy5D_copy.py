...
def generate_data(config_json: dict, used_idx_bkg, used_idx_sig):
    ...
    # if used_idx_bkg is not None:
    #     mask = torch.ones(REF_FEAT.shape[0], dtype=torch.bool)
    #     mask[used_idx_bkg] = False
    #     REF_FEAT = REF_FEAT[mask]
    
    # if used_idx_sig is not None:
    #     mask = torch.ones(DATA_FEAT.shape[0], dtype=torch.bool)
    #     mask[used_idx_sig] = False
    #     DATA_FEAT = DATA_FEAT[mask]

    # EXTRACT RANDOM EVENTS FROM THE INPUT FILTERED DATASET
    # indeces for the Reference and the BKG dataset
    # idxs    = torch.multinomial(torch.ones(N_EVENTS_R), num_samples=(config_json["N_Ref"] + config_json['N_Bkg']), replacement=False)
