...
def main(args, config_json):
    ...
    ref_size, data_size = load_dataset(config_json["DATA_PATH"], config_json["TENSOR_PATH"], config_json["NP"])
    used_idx_bkg = np.array(())
    used_idx_sig = np.array(())
    ...
    if args.local:
        ...
        for i in range(args.toys):
            ...
            ''' 
            CREATE DATASET AND SAVE THE PSEUDO-DATA INDEX
            '''
            idx_bkg, idx_sig = generate_data(config_json, used_idx_bkg, used_idx_sig)
            used_idx_bkg = np.concatenate([used_idx_bkg,idx_bkg])
            used_idx_sig = np.concatenate([used_idx_sig,idx_sig])
            ...
            # os.system(f"python {os.getcwd()}/{args.pyscript} -j {config_json["jsonfile"]} -i {i} --debug {False}")
            
            time_stop = time()
            f = open("/home/ubuntu/NPLM.torch/notebooks/tensor/timing.txt", "a")
            f.write(f'\t{time_stop - time_start}\n')
            f.close()
...
if __name__ == "__main__":
    ...
    c_json = {
        "DATA_PATH"         : "/home/ubuntu/NPLM.torch/notebooks/data/", ## where the data files are
        "TENSOR_PATH"       : "/home/ubuntu/NPLM.torch/notebooks/tensor/data_tensor.pt", ## where to save/read the tensor containint all the data
        'DATASET_PATH'      : "/home/ubuntu/NPLM.torch/notebooks/tensor/dataset.pt", ## where to save/read the dataset tensors
        "NP"                : "Z",
        "N_Ref"             : 100_000,
        "N_Bkg"             : 20_000,
        "N_Sig"             : 0,
